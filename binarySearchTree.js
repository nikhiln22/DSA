...
class BinarySearchTree {
  ...
  isEmpty() {
    // method to check whether the bst is empty or not
    return this.root === null;
  }

  insert(value) {
    // inserting an new node to the BST , if the bst is empty the new node added will be made as an root node
  ...
  insertNode(root, newNode) {
    // method to check where the newnode should be inserted either as the leftchild or as the rightside
  ...
  search(root, value) {
    // method to check whether the value is there or not in the bst
    if (!root) {
      return false;
    } else {
      if (root.value === value) {
  ...
  preOrder(root) {
    // preorder traversal in the depth first traversal
    if (root) {
  ...
  inOrder(root) {
    //  inorder traversal in the depth first traversal
    if (root) {
  ...
  postOrder(root) {
    //  postorder traversal in the depth first traversal
    if (root) {
  ...
  levelOrder() {
    //  breadth first search traversal using Queue Data Structure
    const queue = [];
  ...
  min(root) {
    // finding the min value from the tree
    if (!root.left) {
      return root.value;
    } else {
  ...
  max(root) {
    // finding the max value from the tree
    if (!root.right) {
      return root.value;
    } else {
  ...
  deleteNode(root, value) {
    // helper function for deleting the node from the bst
    if (root === null) {
      return root;
    }
    if (value < root.value) {
  ...
  isBst(root, min, max) {
    // validating whether it is an binary search tree
    if (!root) {
      return true;
    }
  ...
  printLeafNodes(root) {
    // displaying the leaf nodes in an binary search tree
    if (root === null) {
      return;
    }
  ...
  removeDuplicates() {
    // remove duplicates from an binary search tree
    this.removeDuplicatesHelper(this.root, new Set());
  }

  removeDuplicatesHelper(root, valueSet) {
    // helper function for removing the duplicates from the binary search tree
    if (root === null) return;
...
class BinarySearchTree {
  ...
  countLeftNodes(root) {
    ...
    return this.inOrder(root.left).length;
  }
}
...
class BinarySearchTree {
  ...
  countRightNodes(root) {
    ...
    return this.inOrder(root.right).length;
  }
}
...
console.log(bst.countRightNodes(bst.root));

// - printing the left subtree in bst
...
bst.printLeftSubtree(bst.root);

// - printing the right sub tree
...
bst.printRightTree(bst.root);

// checking whether an bst is an avl tree
...
class BinarySearchTree {
  ...
  isAvl(root) {
    ...
    return this.isAvl(root.right) && this.isAvl(root.left);
  }
}

const bst = new BinarySearchTree();

[30, 20, 40, 10, 25, 35, 50].forEach((v) => bst.insert(v));

console.log(bst.isAvl(bst.root));

// checking whether an bst is an perfect tree
...
class Node {
  ...
  countNodes(root) {
    ...
    let leftNodes = this.countNodes(root.left);
    let rightNodes = this.countNodes(root.right);
